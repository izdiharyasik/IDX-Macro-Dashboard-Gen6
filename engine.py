...
def get_timing_model_signal(details, scores, flow_data=None, results=None,
                            cpi_yoy=3.3, credit_spread=4.0, accounting_risk=False):
    """
    Reference model inspired by the 5-box framework from user-provided transcript:
    1) VIX panic (>30)
    2) Fed not hiking (proxy via US10Y pressure easing)
    3) Deleveraging visible (proxy via recent Nasdaq drawdown)
    4) Clear leading sector (positive money-flow leader)
    5) Leader earnings healthy (proxy via positive fundamental quality in top names)
    Plus 3 trap filters: credit spread, inflation, accounting trust risk.
    """
    ...
    # 1) VIX > 30
    try:
        vix_raw = float(details.get("VIX", {}).get("value", "0").replace(",", ""))
    except Exception:
        vix_raw = 0.0
    vix_panic = vix_raw >= 30

    # 2) Fed not hiking (proxy)
    fed_supportive = scores.get("US10Y", 0) > -0.25

    # 3) Deleveraging proxy (recent Nasdaq weakness)
    nas_20d = str(details.get("Nasdaq", {}).get("trend", {}).get("20d", ""))
    deleveraging = "▼" in nas_20d or scores.get("Nasdaq", 0) < 0

    # 4) Clear leading sector
    lead_sector = "N/A"
    lead_score = 0.0
    if flow_data:
        ranked = sorted(flow_data.items(), key=lambda x: x[1].get("score", -999), reverse=True)
        if ranked:
            lead_sector = ranked[0][0]
            lead_score = float(ranked[0][1].get("score", 0))
    sector_lead = lead_score > 0

    # 5) Earnings strength proxy from analyzed names
    if results:
        top = sorted(results, key=lambda x: x.get("composite", 0), reverse=True)[:max(3, len(results)//4)]
        avg_fund = float(np.mean([r.get("fundamental", 0) for r in top])) if top else 0.0
    else:
        avg_fund = 0.0
    earnings_ok = avg_fund >= 0

    boxes = {
        "VIX Panic (>30)": vix_panic,
        "Fed Not Hiking (proxy)": fed_supportive,
        "Deleveraging (proxy)": deleveraging,
        "Leading Sector Present": sector_lead,
        "Leader Earnings Healthy (proxy)": earnings_ok,
    }
    passed = sum(1 for v in boxes.values() if v)
    base_prob = (passed / 5.0) * 82.0

    trap_flags = {
        "Credit Spread Stress (>6%)": float(credit_spread) > 6.0,
        "Inflation Too Hot (>3.5%)": float(cpi_yoy) > 3.5,
        "Accounting Trust Risk": bool(accounting_risk),
    }
    trap_penalty = sum(20 for v in trap_flags.values() if v)
    probability = int(np.clip(round(base_prob - trap_penalty), 0, 95))

    if passed == 5 and not any(trap_flags.values()):
        signal = "BUY WINDOW"
    elif passed >= 3 and probability >= 45:
        signal = "WATCH / STAGGERED BUY"
    else:
        signal = "NO-GO / WAIT"
    ...
    return {
        ...
        "avg_fundamental_proxy": round(avg_fund, 2),
